...
bool Line::isParallel_cp(const Line& otherLine, double tol) const 
{
    float cross1 = v.x * otherLine.v.y - v.y * otherLine.v.x;
    float cross2 = v.x * otherLine.v.y - v.y * (-otherLine.v.x);
    //std::cout << cross << std::endl;
    return std::fabs(cross1) < tol || std::fabs(cross2) < tol;
}
