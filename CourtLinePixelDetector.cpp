...
Mat CourtLinePixelDetector::run(const Mat& frame)
{
  ...
  Mat grayImage;
  cv::cvtColor(trans, grayImage, cv::COLOR_BGR2GRAY);

  TimeMeasurement::start("\tdetectLinePixels");
  Mat image = detectLinePixels(grayImage);
  TimeMeasurement::stop("\tdetectLinePixels");

  TimeMeasurement::start("\tfilterLinePixels");
  image = filterLinePixels(image, grayImage);
...
uchar CourtLinePixelDetector::isLinePixel(const Mat& image, unsigned int x, unsigned int y)
{
  ...
  //For thick line
  topValue = image.at<uchar>(y-10, x);
  lowerValue = image.at<uchar>(y+10, x);
  leftValue = image.at<uchar>(y,x-10);
  rightValue = image.at<uchar>(y,x+10);
...
Mat CourtLinePixelDetector::colorTransfer(const Mat& frame)
{
    cv::Mat imgtrans = Mat::zeros(frame.size(), frame.type());
 
    std::cout << " Basic Linear Transforms " << std::endl;
    std::cout << "-------------------------" << std::endl;
    std::cout << "* Enter the alpha value [1.0-3.0]: "; std::cin >> parameters.alpha;  /*< Manual contrast control */
    std::cout << "* Enter the beta value [0-100]: ";    std::cin >> parameters.beta;   /*< Manual brightness control */
 
    for( int y = 0; y < frame.rows; y++ ) {
        for( int x = 0; x < frame.cols; x++ ) {
            for( int c = 0; c < frame.channels(); c++ ) {
                imgtrans.at<Vec3b>(y,x)[c] =
                  saturate_cast<uchar>( parameters.alpha * frame.at<Vec3b>(y,x)[c] + parameters.beta );
            }
        }
    }
    ...
    if (debug)
    {
        displayImage(windowName, imgtrans);
    }
